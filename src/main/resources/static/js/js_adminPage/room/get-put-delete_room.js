...
function start(){
  getCourses(renderCourses);							//thực hiện hàm getCourses. Hàm getCourse sẽ trả về callback hàm renderCourses
}
start();

//Functions Get All
/*function getCourses(callback){					
  fetch(courseApi)										//lấy data từ db ra dưới dạng json
...
function getCourses(callback, categoryCode = '') {
  var url = courseApi;
  if (categoryCode) {
    url = roomByCategoryApi + '?categoryCode=' + categoryCode;
  }
...
function handleDeleteCourse(id) {
    var options = {
        ...
        body: JSON.stringify([id]) 						// Gửi mảng ID trong body yêu cầu
    };
	fetch(courseApi + '/' + id, options)				//Lấy id xóa ở frontend
	   .then(function(response) {
          return response.json();
	   })
	   .then(function() {
	      var courseItem = document.querySelector('.course-item-' + id);
	      if(courseItem) {							   //nếu phần tử tồn tại thì xóa khỏi DOM thì không phải gọi API
		     courseItem.remove();
	      }
	      //getCourses(renderCourses);					//tải lại trang sau khi xóa
	   })
    fetch(courseApi, options)							//Lấy [id] xóa ở database
        .then(function(response) {
            return response.json();
        })
}
...
function renderCourses(data){							//courses là 1 mảng các đối tượng json đc lấy ra từ .then trc đó
  var listCoursesBlock = document.querySelector('#list-courses');		//lấy phần tử ul trong html
  ...
  var htmls = courses.map(function(course){				//sd map() để thay đổi giá trị các phần tử trong mảng
  														//class="course-item-${course.id}" dùng để xác định đúng phần tử li muốn xóa
  														
  	  //var imageUrl = course.imageUrl || ''; <img id="custom-img" src="${imageUrl}"> 			// Lấy URL đầu tiên
	  //console.log(course.imageUrl);
	  return `<tr class="course-item-${course.id}">
                <td data-label="Tour Package">
                  <div class="product-name">
                     <div class="img">
                        <img id="custom-img" src="${course.imageUrl}">
                     </div>
                     <div class="product-content">
                        <p id="title-${course.id}">${course.title}</p>
                        <p id="price-${course.id}">$${course.price} night</p>
                        <p id="roomService-${course.id}">${course.roomService}</p>
                        <p>
                           ...
                           </svg> <span id="category-${course.id}">${course.categoryName}</span>
                        </p>
                      </div>
                      <div class="product-update">
                         <button data-bs-toggle="modal" data-bs-target="#user-login" onclick="handleUpdateCourse(${course.id})">
...
function handleUpdateCourse(id) {    
    ...
    fetch(courseApi + '/' + id)
        ...
        .then(course => {
            ...
            document.querySelector('input[name="shortDescription"]').value = course.shortDescription;
            document.querySelector('textarea[name="content"]').value = course.content;
            document.querySelector('input[name="price"]').value = course.price;
            document.querySelector('input[name="roomService"]').value = course.roomService;
            document.querySelector('input[name="roomPoint"]').value = course.roomPoint;           
            const categoryCode = convertCategoryNameToValue(course.categoryName);				// Chuyển đổi từ categoryName sang categoryCode và thiết lập giá trị cho thẻ select
	...
	updateBtn.onclick = function() {
		...
		var roomPoint = document.querySelector('input[name="roomPoint"]').value;
